...
<div class="admin-card">
    <div class="admin-card__header">
        ...
        <div>
            @if($blogPost->status==='published')<a href="{{ route('blog.show', $blogPost->slug) }}" class="admin-btn admin-btn--secondary admin-btn--sm" target="_blank">View Public</a>@endif
            <a href="{{ route('admin.blog-posts.edit', $blogPost) }}" class="admin-btn admin-btn--primary admin-btn--sm">Edit</a>
        </div>
    </div>
...
<form action="{{ route('admin.blog-posts.destroy', $blogPost) }}" method="POST" onsubmit="return confirm('Delete?')">@csrf @method('DELETE')<button type="submit" class="admin-btn admin-btn--danger">Delete</button></form>
@endsection
